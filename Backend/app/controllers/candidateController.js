...
class CandidateController {
  ...
  async submitAssessment(req, res) {
    try {
      ...
      const assessment =
        await Assessment.findById(assessmentId).populate("questions");
      if (!assessment) {
        return res.status(404).json({
          success: false,
      ...
      for (const item of userAnswers) {
        ...
        if (questionObj) {
          ...
          if (
            String(questionObj.correctAnswer).trim().toLowerCase() ===
            String(item.selectedAnswer).trim().toLowerCase()
          ) {
            isCorrect = true;
  ...
  async downloadCertificate(req, res) {
    try {
      ...
      if (!attempt) {
        return res
          .status(404)
          .json({ success: false, message: "Test attempt record not found!" });
      }
      ...
      if (!attempt.isPassed) {
        return res
          ...
          .json({
            success: false,
            message: "Certificate only available for passed candidates!",
          });
      }

      const pdfBuffer = await pdfService.generateCertificate(
        attempt.candidateId.name,
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=Certificate_${attempt._id}.pdf`,
      );
  ...
  // Candidate Dashboard Data (My Assessments & Certificates)
  async getMyDashboard(req, res) {
    try {
      const candidateId = req.user._id;

      // ১. ক্যান্ডিডেটের সম্পন্ন করা পরীক্ষা এবং সার্টিফিকেট
      const completedAttempts = await TestAttempt.find({ candidateId })
        .populate(
          "assessmentId",
          "title description durationMinutes totalMarks passingMarks",
        )
        .sort({ completedAt: -1 });

      // ২. পাবলিশড অ্যাসাইনমেন্টের লিস্ট (যেগুলো ক্যান্ডিডেট দিতে পারবে)
      const availableAssessments = await Assessment.find({
        status: "published",
        isPublic: true,
      }).select(
        "title description durationMinutes totalMarks passingMarks createdAt",
      );
